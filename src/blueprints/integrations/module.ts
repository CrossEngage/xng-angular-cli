...
import { SharedIntegrationsModule } from 'app/modules/integrations/shared-integrations/shared-integrations.module';
<% if (generateSetup) { %>
import { <%= upperCamelCaseFn(provider) %><%= upperCamelCaseFn(channelType) %>SetupComponent }       from './<%= provider %>--<%= channelType %>-setup.component';<% } %><% if (generateMessageEdit) { %>
import { <%= upperCamelCaseFn(provider) %><%= upperCamelCaseFn(channelType) %>MessageEditComponent } from './<%= provider %>--<%= channelType %>-message-edit.component';<% } %><% if (generateMessageView) { %>
import { <%= upperCamelCaseFn(provider) %><%= upperCamelCaseFn(channelType) %>MessageViewComponent } from './<%= provider %>--<%= channelType %>-message-view.component';<% } %><% if (generateService) { %>
